...
#pragma unroll
    ...
    void grid_nms(std::vector<pyramid_t> pyramid,
                  ...
                  cudaStream_t stream)
    {
        const int horizontal_cell_num = (pyramid[0].image_width % CELL_SIZE_WIDTH == 0) ? pyramid[0].image_width / CELL_SIZE_WIDTH : pyramid[0].image_width / CELL_SIZE_WIDTH + 1;
        const int vertical_cell_num = (pyramid[0].image_height % CELL_SIZE_HEIGHT == 0) ? pyramid[0].image_height / CELL_SIZE_HEIGHT : pyramid[0].image_height / CELL_SIZE_HEIGHT + 1;
        for (std::size_t level = 0; level < pyramid.size(); level++)
        {
            const int cell_size_width_level = (CELL_SIZE_WIDTH >> level);
            const int cell_size_height_level = (CELL_SIZE_HEIGHT >> level);
